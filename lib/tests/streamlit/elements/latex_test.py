...
class LatexTest(DeltaGeneratorTestCase):
    ...
    def test_latex_width(self, width_value, expected_field, expected_value):
        ...
        c = self.get_delta_from_queue().new_element.markdown
        self.assertEqual(c.body, "$$\nax^2 + bx + c = 0\n$$")
        self.assertTrue(c.width_config.WhichOneof("width_spec") == expected_field.value)
        if expected_field == WidthConfigFields.PIXEL_WIDTH:
            self.assertEqual(c.width_config.pixel_width, expected_value)
        elif expected_field == WidthConfigFields.USE_STRETCH:
            self.assertTrue(c.width_config.use_stretch)
        else:
            self.assertTrue(c.width_config.use_content)
